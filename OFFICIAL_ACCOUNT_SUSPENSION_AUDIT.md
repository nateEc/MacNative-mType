...
封禁期间服务端拒绝公开资料更新和完整账户重置；登录、同步、本机练习、已有成绩、成绩读取、密码、认证方式及删号保持可用。撤销后公开资料恢复完整展示，历史成绩继续保留；但与固定参考一致，封禁前及封禁期间的日榜与周 XP 滚动项不会回流，只有撤销后由服务端新接受的成绩重新进入这些榜单。该边界以服务端接受时间而不是可由客户端提交的结束时间判断。状态同时回传给账户本人、公开资料和部署方私有审核队列；老服务响应及旧 JSON 状态未出现该字段时，原生客户端和服务端均安全地当作未封禁。
