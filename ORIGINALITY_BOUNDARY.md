...
同一模式还会对项目中的 PNG/JPEG/GIF/WebP/SVG/PDF/图标、WOFF/TTF/OTF 字体及常见音频文件，与固定参考检出中的同类文件比较 SHA-256。只要存在完全相同的字节就失败，且仅报告重复哈希数量。当前 Typebar 没有此类打包资产；该检查确保后续新增的自制或已授权资产不会悄悄变成参考副本。
