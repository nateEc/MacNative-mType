...
用户主动粘贴的网页测试设置链接同样只是一段离线载体：Typebar 对 HTTP(S) URL 的 `testSettings` 查询项进行有大小上限的 URI 压缩解码，并在完整 JSON 元组、原生语言、模式、数值、难度和 funbox 名称全部校验后才产生新的本地测试配置。主机不被视为服务身份，也不会被请求或代理；链接中的外部引语 ID 也绝不用于取得或伪造参考引语内容。该解码器是独立的 Swift 实现，只兼容公开数据格式，不包含参考项目代码或运行时依赖。
