...
| `configs`、`presets` | 跨设备保存、读取、编辑或删除配置与预设 | `DataTransfer.swift`、`TestPresets.swift`、`RemoteAccount.swift` 与 `GET/POST /v1/sync` 传输版本化 Typebar 归档；本机测试预设从 v7 起携带稳定 UUID 与显式删除标记，阻止旧设备快照复活；冲突显式保留本机标量并另存冲突副本。 |
