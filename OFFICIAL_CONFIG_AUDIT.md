...
| `layout` | `KeyboardInputLayout` | 部分；系统输入源为默认，31 种原创物理布局和用户自写四行布局（可选等长 Shift 图例）的基础映射可显式模拟；Polish (Programmers) 额外有独立的 Option/Shift+Option 字符层，官方全部命名布局尚未覆盖。 |
