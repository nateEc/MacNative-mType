...
2026-09-21 审计更正：固定参考提交 `91bd24bb8513785c7364cbea29296ff7adafac41` 的生成器将全部日语变体（包括罗马字）归入 `japanese` 标点分支，并以普通提交空格分词；日语罗马字已按此实现。中文与假名当前仍使用 Typebar 已有的无空格边界、逐词统计与复盘路径；将以独立迁移契约把这三个耦合面切换到参考的空格语义，避免在标点修复中破坏进行中的会话或历史结果。
...
2026-09-09 当前更正：有效快照现为 232 种单语与 153 种默认／自选 LTR 多语候选。简体中文 1k／5k／10k／50k 已提升为四个独立入口，以原创确定性按需 CJK 索引词流保持 1,000／5,000／10,000／50,000 个唯一项及固定长度、Unicode 数字、标点、大写、非 ASCII 和纯 CJK 结构；普通无空格出题不物化整表。机器总账为 446／302／144／0，不导入参考词值、代码或资产。

2026-09-09 当前更正：有效快照现为 236 种单语与 153 种默认／自选 LTR 多语候选。繁體中文 1k／5k／10k／50k 已提升为四个独立入口，以原创确定性按需繁体 CJK 索引词流保持 1,000／4,991／9,974／49,925 个唯一项及固定长度、Unicode 数字、标点、大写、非 ASCII、纯 CJK 和数字标点交叠结构；普通无空格出题只解析本次提示。机器总账为 446／306／140／0，不导入参考词值、代码或资产。
