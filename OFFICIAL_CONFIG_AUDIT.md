...
| `customLayoutfluid` | `layoutFluidLayouts` | 已映射；官方上限 15，当前 123 个原生内置布局可任选至多 15 个进入原生序列。 |
...
- 完整客户端 `swift test` 通过 330 项、独立 Vapor 服务 `swift test` 通过 67 项；测试前后 `pgrep -ax Typebar` 均无输出，未启动图形应用。
...
- 官方布局矩阵测试验证固定源码 239 个名称完整且唯一、118/3/118 三类状态数量守恒、官方名称引用有效，并确保每个原生目标都能解析为当前 `KeyboardLayout`。
