...
- 完整 `swift test` 已通过 247 项测试；测试前 `pgrep -x Typebar` 无输出，未启动图形应用。
- 独立 Vapor 服务的 `swift test` 已通过 64 项测试，其中覆盖 GitHub/Google/Discord OAuth 的 PKCE 授权 URL、一次性 state、原生回调、注册/关联、提供商匹配重新验证、安全移除、Discord 头像公开隐私、原创公开徽章及公开连续练习/开始次数隐私；测试前 `pgrep -x Typebar` 无输出，未启动图形应用。
