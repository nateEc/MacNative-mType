...
| `layout` | `KeyboardInputLayout` | 部分；系统输入源为默认，40 种原生物理布局和用户自写四行布局（可选等长 Shift 图例）的基础映射可显式模拟；Arabic (101)/(102) 与 Persian (Farsi) 可保留单键完整文本输出，Polish (Programmers)、Arabic (macOS) 与 Urdu Phonetic 有独立 Option 层，官方全部命名布局尚未覆盖。 |
