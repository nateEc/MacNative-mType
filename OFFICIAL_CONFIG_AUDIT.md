...
- 完整客户端 `swift test` 通过 350 项、独立 Vapor 服务 `swift test` 通过 67 项；测试前后 `pgrep -ax Typebar` 均无输出，未启动图形应用。
...
- 官方布局矩阵测试验证固定源码 239 个名称完整且唯一、213/3/23 三类状态数量守恒、官方名称引用有效，并确保每个原生目标都能解析为当前 `KeyboardLayout`。
