...
| `customLayoutfluid` | `layoutFluidLayouts` | 部分；官方上限 15，当前原创内置可模拟布局为 14。 |
...
| `layout` | `KeyboardInputLayout` | 部分；系统输入源为默认，14 种原创物理布局可显式模拟；官方全部命名布局尚未覆盖。 |
...
1. 官方 239 份命名 `keymapLayout` 资产与输入模拟尚未全部覆盖。当前策略优先使用 macOS 当前输入源（普通、Shift、Option、Shift+Option 四层标签）、14 个原创内置模拟布局和用户自写视觉图，避免复制资产；仍需继续扩展原创输入映射覆盖。
...
- 完整 `swift test` 已通过 237 项测试；测试前 `pgrep -x Typebar` 无输出，未启动图形应用。
