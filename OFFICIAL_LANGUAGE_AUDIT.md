...
446 个官方 ID 当前严格分区为：238 个 Typebar 独立原生选择，以及 208 个数字词表规模变体（由同语言的已有原生选择表达，但没有对应的独立规模选项）。当前没有未映射 ID；由于 208 个规模变体仍非独立入口，不能宣称官方配置选择一一等价。

## 已覆盖的原生语言面

当前语言目录：168 个可单独练习的语言或书写方式、70 个代码选择和 2 个混合入口。168 个单语入口均支持 Typebar 自有引语；最新增加五个 English 数字规模入口。较早逐项补充中的数量只记录当时状态，当前数字以本段及文末最新更正为准。
...
- `testPinnedOfficialLanguageCoverageIsPartitionedAndResolvable` 固定 446／238／208／0 守恒关系、分区互斥、每个映射可解析以及 238 个非混合原生选择的一一覆盖；生成器还会拒绝错误参考提交和意外数量变化。
...
- 2026-09-09 更正：当前单语总数为一百六十八种，默认／自选 LTR 多语候选仍为一百五十三种。固定 English 五个数字规模以聚合方式确认实际数量分别为 1,000／5,000／9,944／24,141／450,029，并确认长度、大小写、符号、`noLazyMode` 和词频排序字段。Typebar 以五套惰性初始化的原创确定性词流提供独立 ID 和菜单选择，贯通四档自有引语及全部数据面；大型规模不重复进入默认混排。机器总账现为 446／238／208／0，不复制参考词值、代码或资产。
