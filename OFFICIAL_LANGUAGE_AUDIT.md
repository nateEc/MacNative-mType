...
446 个官方 ID 当前严格分区为：446 个 Typebar 独立原生选择、0 个兼容代指和 0 个未映射 ID。该一一覆盖仅说明每个固定配置 ID 都有独立的原生选择与行为契约；Typebar 不复制官方词值、引语、代码、资产或实现，因此不把聚合形状覆盖误报为内容相同。

## 已覆盖的原生语言面

当前语言目录：376 个可单独练习的语言或书写方式、70 个代码选择和 2 个混合入口。376 个单语入口均支持 Typebar 自有引语；最新补齐 Amharic、Armenian、Western Armenian、Belarusian Lacinka、Hawaiian、Japanese Romaji、Klingon、Oromo、Shona、Tibetan、English Five-Letter 与 Xhosa 的十四个剩余规模入口。较早逐项补充中的数量只记录当时状态，当前数字以本段及文末最新更正为准。
...
- `testPinnedOfficialLanguageCoverageIsPartitionedAndResolvable` 固定 446／446／0／0 守恒关系、分区互斥、每个映射可解析以及 446 个非混合原生选择的一一覆盖；生成器还会拒绝错误参考提交和意外数量变化。
...
- 2026-09-09 更正：当前单语总数为二百五十五种，默认／自选 LTR 多语候选仍为一百五十三种。固定 `swiss_german_1k`／`2k` 以可见 `ß`→`ss` 后聚合确认 1,000／2,000 个唯一项、2–16／2–29 字符及逐档大小写、标点、空格、数字、非 ASCII 和交叠结构；源码按 Swiss German 组复用 German 引语、转换可见文本并拒绝社区投稿。固定 `afrikaans_1k`／`10k` 确认 1,000／8,164 个唯一项、2–19／2–22 字符及逐档大小写、标点、非 ASCII 和交叠结构，仅 1k 定义 `noLazyMode: true`。Typebar 以两套原创参数化按需索引词流提供四个独立入口，保持对应在线、朗读、四档引语和全部数据面，不加入多语混排；Swiss German 两档拒绝投稿但接受成绩与排行榜。与四个固定可见词表的精确交集均为零，机器总账现为 446／325／121／0。
- 2026-09-09 更正：当前单语总数为二百五十九种，默认／自选 LTR 多语候选仍为一百五十三种。固定 `italian_1k`／`7k`／`60k`／`280k` 以聚合方式确认 1,159／7,154／60,442／279,833 个唯一项、1–14／1–18／1–18／1–25 字符及逐档大小写、标点、符号和非 ASCII 数量；四项都仅定义 `bcp47: it-IT`，未定义 RTL、连写、`noLazyMode` 或词频排序。Typebar 以一套原创参数化按需索引词流提供四个独立入口，接入 LTR 空格词界、`it`／`it-IT`、四档自有引语和全部数据面，不加入多语混排；与四个固定词表逐档精确交集为零，机器总账现为 446／329／117／0。
- 2026-09-09 更正：当前单语总数为二百七十一种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Esperanto 标准、X-sistemo 与 H-sistemo 的 1k／10k／25k／36k 共十二项，以聚合方式确认逐档实际数量、1–15／17／18／40 字符边界及非 ASCII 数量，不读取词值。Typebar 以一套原创参数化按需索引词流提供十二个独立入口，按书写族继承四档自有引语，标准书写保留简化输入，X/H 禁用简化输入，并精确保持各档 Zipf 元数据；全部使用 LTR 空格词界与 `en`／`en-US` 缺省路径，贯通数据面且不加入多语混排。与十二个固定词表逐档精确交集为零，机器总账现为 446／341／105／0。
- 2026-09-09 更正：当前单语总数为二百七十九种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Greek 与 Greeklish 的 1k／5k／10k／25k 共八项，以聚合方式确认逐档实际数量、1–16／20／21 字符边界、全 Greek 非 ASCII 与 Greeklish 少量非 ASCII 异常，不读取词值。Typebar 使用两套原创按需索引词流提供八个独立入口；Greek 保留简化输入与 Zipf 未知语义，Greeklish 禁用简化输入、明确不支持 Zipf 且继续阻止百科希腊原文替换 Latin 练习。八档使用 LTR 空格词界与 `el`／`el-GR`，按书写族继承四档自有引语，贯通数据面且不加入多语混排。与八个固定词表逐档精确交集为零，机器总账现为 446／349／97／0。
- 2026-09-09 更正：当前单语总数为二百八十七种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Qırımtatarca 与 Къырымтатарджа 的 1k／5k／10k／15k 共八项，以聚合方式确认逐档 1,000／5,000／10,000／15,082 个唯一项、1 字符下限、17–20 字符上限，以及脚本、大小写、标点、非 ASCII 与交叠结构，不读取词值。Typebar 使用两套原创按需索引词流提供八个独立入口；全部禁用简化输入，使用 LTR 空格词界与 `crh`／`crh-CRH`，按书写族继承四档自有引语，贯通数据面且不加入多语混排。与八个固定词表逐档精确交集为零，机器总账现为 446／357／89／0。
- 2026-09-09 更正：当前单语总数为二百九十五种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Occitan 与 Taqbaylit 的 1k／2k／5k／10k 共八项，以聚合方式确认逐档 1,000／2,000／5,000／10,000 个唯一项、长度、非 ASCII 及 Kabyle 标点与非 ASCII 交叠数量，不读取词值。Typebar 使用两套原创按需索引词流提供八个独立入口；Occitan 保持 `oc`／`oc-FR` 与 Zipf 未知语义，Kabyle 保持 `kab` 与明确不支持 Zipf，八档均保留简化输入、按语言族继承四档自有引语、贯通数据面且不加入多语混排。与八个固定词表逐档精确交集为零，机器总账现为 446／365／81／0。
- 2026-09-09 更正：当前单语总数为三百零三种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Hebrew 1k／5k／10k、Persian 1k／5k／20k 与 Urdu 1k／5k 共八项，以聚合方式确认 1,000／5,000／10,000、1,000／5,000／21,715 及 934／4,981 个唯一项、逐档长度、全非 ASCII、多词、标点、组合标记与格式字符结构，不读取词值。Typebar 使用三套原创按需索引词流提供八个独立入口；全部使用 RTL 空格词界及连写保护，Hebrew 规模档保留简化输入而 Persian／Urdu 禁用，分别继承三族四档自有引语、贯通数据面且不加入多语混排。与八个固定词表逐档精确交集为零，机器总账现为 446／373／73／0。
- 2026-09-09 更正：当前单语总数为三百零八种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Tamil 1k、Telugu 1k、Bangla 10k、Hindi 1k 与 Gujarati 1k 以聚合方式确认 951／901／9,734／999／1,004 个唯一项、标量／字素长度、组合标记、Bangla 标点及 Telugu／Gujarati 内部空格结构，不读取词值。Typebar 使用五套原创按需索引词流提供独立入口；五档均使用 LTR 空格词界、连写保护并禁用简化输入，Tamil 1k 保持 Zipf 支持，其余为未知，分别继承各族四档自有引语、贯通数据面且不加入多语混排。与五个固定词表逐档精确交集为零，机器总账现为 446／378／68／0。
...
- 2026-09-09 更正：当前单语总数为三百五十三种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Vietnamese 1k／5k、Pinyin 1k／10k、Hausa 1k、Bemba 1k／10k、Catalan 1k 与 Frisian 1k 以聚合方式确认逐档实际数量、长度、大小写、标点、多级内部空格、非 ASCII 与交叠结构，不读取词值。Typebar 使用原创按需索引词流提供九个独立入口；九档保留简化输入，Bemba 两档明确不支持 Zipf，其余为未知，按各自 BCP-47 或缺省区域规则接入在线／朗读路径，分别继承各族四档自有引语、贯通数据面且不加入多语混排。普通出题不物化整表，与九个固定词表逐档精确交集为零；机器总账现为 446／423／23／0。
- 2026-09-09 更正：当前单语总数为三百六十二种，默认／自选 LTR 多语候选仍为一百五十三种。固定 Serbian Latin 10k、Serbian 10k、Bulgarian 1k、Bulgarian Latin 1k、Bosnian 4k、Albanian 1k 与 Macedonian 1k／10k／75k 以聚合方式确认逐档实际数量、长度、拉丁或西里尔脚本、大小写、标点、多级内部空格与交叠结构，不读取词值。Typebar 使用原创按需索引词流提供九个独立入口；Serbian、Bulgarian 和 Macedonian 指定规模禁用简化输入，Bulgarian 两档明确不支持 Zipf，其余为未知，按各自 BCP-47 或缺省区域规则接入在线／朗读路径，分别继承各族四档自有引语、贯通数据面且不加入多语混排。普通出题不物化整表，与九个固定词表逐档精确交集为零；机器总账现为 446／432／14／0。
