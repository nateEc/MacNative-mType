...
446 个官方 ID 当前严格分区为：321 个 Typebar 独立原生选择，以及 125 个数字词表规模变体（由同语言的已有原生选择表达，但没有对应的独立规模选项）。当前没有未映射 ID；由于 125 个规模变体仍非独立入口，不能宣称官方配置选择一一等价。

## 已覆盖的原生语言面

当前语言目录：251 个可单独练习的语言或书写方式、70 个代码选择和 2 个混合入口。251 个单语入口均支持 Typebar 自有引语；最新增加 Bahasa Indonesia 1k／10k 与 کوردی ناوەندی 2k／4k 数字规模入口。较早逐项补充中的数量只记录当时状态，当前数字以本段及文末最新更正为准。
...
- `testPinnedOfficialLanguageCoverageIsPartitionedAndResolvable` 固定 446／321／125／0 守恒关系、分区互斥、每个映射可解析以及 321 个非混合原生选择的一一覆盖；生成器还会拒绝错误参考提交和意外数量变化。
...
- 2026-09-09 更正：当前单语总数为二百三十八种，默认／自选 LTR 多语候选仍为一百五十三种。固定 `azerbaijani_1k` 以聚合方式确认 989 个唯一项、2–7 字符、668 个非 ASCII 条目且无大写、数字、标点、空格或符号；元数据仅声明 `az-AZ`。Typebar 以原创按需拉丁索引词流提供独立入口，保持 LTR 空格词界、简化输入、`az` 百科与 `az-AZ` 朗读，贯通四档自有引语及全部数据面，不加入多语混排；与固定词表精确交集为零。机器总账现为 446／308／138／0。

- 2026-09-09 更正：固定 `malagasy_1k` 以聚合方式确认 975 个唯一项、3–23 字符、5 个大写项、148 个标点项和 32 个非 ASCII 项，其中 25 项同时含标点与非 ASCII；元数据仅声明禁用简化输入。Typebar 使用原创按需索引词流、LTR 空格词界、默认 `en` 百科与 `en-US` 朗读，贯通四档自有引语和全部数据面；与固定词表精确交集为零。

- 2026-09-09 更正：当前单语总数为二百四十种，默认／自选 LTR 多语候选仍为一百五十三种。固定 `malay_1k` 以聚合方式确认 1,000 个唯一项、2–16 字符、1 个大写项、12 个标点项和 1 个内部空格项，全部为 ASCII 且三类互不重叠；元数据为 LTR 与 `ms`。Typebar 使用原创按需索引词流、`ms` 百科与 `ms-MY` 朗读，贯通四档自有引语和全部数据面；与固定词表精确交集为零。机器总账现为 446／310／136／0。

- 2026-09-09 更正：当前单语总数为二百四十一种，默认／自选 LTR 多语候选仍为一百五十三种。固定 `mongolian_10k` 以聚合方式确认 9,219 个唯一项、1–18 字符与 897 个大写项，全部为非 ASCII 字母且无标点、空格、数字、符号或组合标记；元数据仅声明禁用简化输入。Typebar 使用原创按需西里尔索引词流、默认 `en` 百科与 `en-US` 朗读，贯通四档自有引语和全部数据面；与固定词表精确交集为零。机器总账现为 446／311／135／0。

- 2026-09-09 更正：当前单语总数为二百四十七种，默认／自选 LTR 多语候选仍为一百五十三种。固定 `ukrainian_1k`／`10k`／`50k` 与 `ukrainian_latynka_1k`／`10k`／`50k` 以聚合方式确认六档实际规模、1–18／1–30／1–31 字符以及各自大小写、标点、数字、非 ASCII 与交叠结构；六档元数据均只声明禁用简化输入。Typebar 使用两套原创参数化按需索引词流，分别继承现有 Ukrainian 与 Latin 书写的 `uk` 百科、`uk-UA` 朗读及四档自有引语，不加入多语混排；与六个固定词表的精确交集均为零。机器总账现为 446／317／129／0。

- 2026-09-09 更正：当前单语总数为二百五十一种，默认／自选 LTR 多语候选仍为一百五十三种。固定 `indonesian_1k`／`10k` 分别确认 1,020／13,769 个唯一 ASCII 项、2–14／2–13 字符及各自大写和标点聚合；1k 的固定 `bcp47` 异常为 `hu-HU`，10k 为 `id-ID`。固定 `kurdish_central_2k`／`4k` 分别确认 1,486／4,256 个唯一项、1–11 字符、4／3 个内部空格项与 RTL、连写、`ckb` 元数据。Typebar 以两套原创参数化按需索引词流提供四个独立入口，保持对应在线、朗读、四档引语和全部数据面，不加入多语混排；与四个固定词表的精确交集均为零。机器总账现为 446／321／125／0。
