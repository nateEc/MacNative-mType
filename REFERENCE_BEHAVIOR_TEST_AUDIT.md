...
| `frontend/__tests__/utils/format.spec.ts`、`frontend/__tests__/utils/misc.spec.ts` | WPM、准确率与计数 | `AppSettings.swift`、`TypingEngine.swift`、`ResultsAnalytics.swift`、`ResultPersistence.swift` 与 `TypingEngineTests.swift`：单位换算后在普通展示中取整；准确率默认向下取整、普通百分比四舍五入；结果页开启小数时保留未取整速度和准确率至两位，并让稳定度也固定两位，且验证 JSON/本机记录往返；计数和结果摘要保持回归。 |
