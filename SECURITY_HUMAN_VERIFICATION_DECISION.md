...
6. 固定参考源码复核确认 `GoogleSignUpModal` 在调用 `Ape.users.create` 前要求 captcha；因此 Typebar 的 OAuth 新用户注册不能因其统一提供方流程而绕开既有 `registration` proof。完整服务端 109 项与原生 653 项串行套件均已通过，且未启动 GUI。
