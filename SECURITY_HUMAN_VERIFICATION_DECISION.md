...
1. 先为五类用途写失败的服务端路由／消费测试：缺证明、错误用途、到期、重放、并发消费、提供方错误、配置不完整和 hostname 不符。
2. 实现最小 actor、Turnstile HTTP 验证器和服务端挑战页面／回调；再运行完整服务端测试。
3. 为原生相对 URL 解析、固定 callback 三元组、取消和“旧服务不要求”写先失败测试；实现认证会话调用和各写请求携带 proof。
4. 以单个串行的完整原生测试验证，再做一次不启动 GUI 的服务端／原生契约测试；配置说明只列环境变量和失败语义，绝不提交真实密钥。
