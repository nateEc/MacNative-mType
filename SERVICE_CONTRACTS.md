...
| 服务公告 | `GET /v1/announcements`、`POST /v1/moderation/announcements`、`DELETE /v1/moderation/announcements/{id}` | 读取公开且不需要账户；每项仅返回 UUID、1–500 字纯文本、等级、置顶标记和发布时间。发布/删除要求部署者显式配置 `TYPEBAR_MODERATION_TOKEN`，客户端以 `X-Typebar-Moderation-Key` 提交；普通公告可仅在当前 Mac 关闭，置顶项不能由客户端关闭，且不会携带账户、提示、输入、成绩、邮箱或令牌（已实现） |
