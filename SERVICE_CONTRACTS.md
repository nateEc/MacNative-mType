...
所有响应使用 JSON。账户管理使用 Typebar 发行的短期访问令牌；成绩上传还可使用受限的开发者密钥。客户端不会发送原始密码以外的敏感本地数据。
...
| 提交结果 | `POST /v1/results` | 接受 Bearer 令牌，或仅限本端点的 `X-Typebar-Access-Key` 开发者密钥；保存具 UUID 的结果，基本范围/时间校验与重复提交幂等；响应包含服务端重算的本次 XP、总 XP 与可选本周 XP 名次（部分实现） |
