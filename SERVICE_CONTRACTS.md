...
| 部署账户治理 | `PATCH /v1/moderation/profiles/{id}/leaderboard-restriction`、`PATCH /v1/moderation/profiles/{id}/display-name-requirement` | 两个路由都要求部署者显式配置的 `TYPEBAR_MODERATION_TOKEN` 与 `X-Typebar-Moderation-Key`，并接受可逆的布尔请求。前者只影响共享排行榜资格；后者立即排除 WPM/XP 的公共、好友和个人榜，并在持久化前拒绝该账户新的成绩提交，直到其提交一个实际不同且有效的显示名。两者不暂停登录、本机练习或同步，不删除历史数据；状态只返回账户本人和私有资料审核队列，旧响应或旧持久化状态缺字段时默认未设置（已实现） |
