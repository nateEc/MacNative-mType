...
final class OfficialLayoutCoverageTests: XCTestCase {
  ...
  func testCurrentLanguageCatalogCountsMatchCompatibilityDocuments() throws {
    ...
    XCTAssertEqual(languages.count, 282)
    XCTAssertEqual(standalone.count, 210)
    ...
    let repositoryRoot = URL(fileURLWithPath: #filePath)
      ...
      .deletingLastPathComponent()
      .deletingLastPathComponent()
    let currentSummary = "当前语言目录：210 个可单独练习的语言或书写方式、70 个代码选择和 2 个混合入口。"
    for name in [
  ...
  func testPinnedOfficialLanguageCoverageIsPartitionedAndResolvable() throws {
    ...
    XCTAssertEqual(fixture.nativeIndependent.count, 280)
    XCTAssertEqual(fixture.nativeRelatedChoice.count, 166)
