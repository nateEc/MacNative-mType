...
账户治理更新：部署者可凭 `TYPEBAR_MODERATION_TOKEN` 和 `X-Typebar-Moderation-Key` 设置或撤销共享排行榜限制，以及独立的显示名整改要求。后者会立即将账户从公共、好友和个人 WPM/XP 榜排除，并在保存前拒绝新的服务端成绩；登录、浏览、同步和本机练习不受影响，历史数据不会删除。只有用户提交一个实际不同且有效的显示名才会自动解除要求；账户重置、仅更新其他资料或重启服务不能绕过。两项状态仅返回账户本人和私有资料审核队列，旧响应或旧持久化数据缺字段时默认未设置；不实现自动重复名检测、姓名历史、改名冷却、原因披露或全账户封禁。
