...
- 当前内置键盘图为一百六十三种：新增 Hands Down Promethium、Statica 3×5、Vestnik、Diktor 与 Diktor Voronov Mod。Promethium 原生显示 `R + 空格` 拇指行并由物理 Space 输入 `r`；五项的 AltGr 未重定义键保持布局 Base/Shift，Statica/Vestnik 另保留特殊 Cyrillic 层，Diktor 两项保留重排数字符号层，不复制、打包或运行参考布局资产
...
- 338 个引擎、内容、存储、偏好设置、预设、统计、归档、CSV 导出、账户响应及官方布局矩阵单元测试
