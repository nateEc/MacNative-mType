...
- 本地历史汇总、WPM 趋势图、近 28 日完成/练习分钟柱状图、可在最近 12 个月（52 周）与有本机记录的自然年份之间切换、带本地化星期/月标记、当前范围完成总数和五档动态强度图例的活动日历与个人最佳标记；色阶从当前可见日的已知完成数（含零）派生，并裁剪极端值，避免一次异常导入压平日常活跃度。可按同类计时/字数设置查看本机个人最佳表。可首次设定并锁定本机统计日分界（−11 至 +12 小时，每 30 分钟），连续天数和两类活动图同步采用该分界；切换活动范围不修改成绩
...
- 558 个引擎、内容、存储、偏好设置、预设、统计、归档、CSV 导出、账户响应、挑战条件、命令目录及官方布局矩阵单元测试
