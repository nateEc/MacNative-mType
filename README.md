...
- 当前内置键盘图为一百一十三种：新增 Capewell Dvorak、Colman、Heart、Klauser 与 Oneproduct。五项均完整覆盖 47 个 ANSI Base/Shift 位置；Capewell Dvorak 保留重排数字行并在精简提示中显示，Klauser 保留分散标点，Oneproduct 保留问号键的反向 `?`/`/` Base/Shift 配对；不复制、打包或运行参考布局资产
...
- 328 个引擎、内容、存储、偏好设置、预设、统计、归档、CSV 导出、账户响应及官方布局矩阵单元测试
