...
- 固定参考版本的 446 个官方语言配置 ID 已有可重新生成的机器总账：238 个对应 Typebar 独立选择，208 个数字词表规模变体由同语言选择表达，当前没有未映射 ID；项目不会把规模变体误报成独立一一等价，也不会为补齐规模而复制官方语料
...
- English 1k／5k／10k／25k／450k 作为五个独立入口使用惰性初始化的原创确定性词流，保持固定实际规模、长度、大小写、符号和词频排序能力，不导入参考词值
