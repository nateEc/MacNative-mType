...
- 原生账户设置可创建、重命名、禁用和删除最多五个 Typebar 开发者密钥；明文仅在创建时可见，服务端只保存哈希。密钥只允许自动化客户端通过 `X-Typebar-Access-Key` 上传成绩，不能读取资料、同步数据或修改账户
- 可保存、应用和删除完整测试预设
...
- 57 个自建服务自动化测试，覆盖账号、密码重置与邮箱验证、一次性重新验证与会话撤销、OAuth 授权码/PKCE/一次性状态、第三方与密码身份关联保护、公开资料字段/隐私与搜索、开发者密钥、排行榜隐身、好友关系、同步、成绩、WPM/XP 排行榜、审核引语/资料举报、社区评分、请求限速与维护模式
