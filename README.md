...
部署者设置 `TYPEBAR_MODERATION_TOKEN` 后，可在原生设置的“审核员工具”发布或删除公告。所有客户端会在启动时公开读取公告：普通公告可仅在当前 Mac 关闭，置顶公告会保留到服务端删除。公告不需要账户，不包含输入、提示、成绩、邮箱或令牌。
...
- 自建服务可发布公开纯文本公告；普通公告仅在当前 Mac 本机关闭，置顶公告持续显示至部署者删除，发布和删除只接受部署审核密钥
- 可保存、应用和删除完整测试预设
