...
- 当前内置键盘图为一百一十八种：新增 Pine、Real、Sertain、CTGAP 与 Graphite。五项均完整覆盖 47 个 ANSI Base/Shift 位置；Real 保留重排数字行并在精简提示中显示，Graphite 显式保留 `'→_`、`,→?`、`-→"` 与 `/→<` 的非标准 Shift 层；不复制、打包或运行参考布局资产
...
- 329 个引擎、内容、存储、偏好设置、预设、统计、归档、CSV 导出、账户响应及官方布局矩阵单元测试
