...
- 原生账户设置可连接自建服务进行注册、密码或 GitHub/Google 登录、会话恢复、密码重置、邮箱验证、资料刷新、第三方方式关联/移除、为第三方账户添加或移除密码方式和退出；访问令牌仅保存在 macOS 钥匙串。重置码仅能使用一次、20 分钟后过期，完成重置会注销该账户所有设备；验证邮件在注册或改邮箱后自动发送，也可手动重发，验证码只能使用一次并在 24 小时后过期
- 可保存、应用和删除完整测试预设
...
- 51 个自建服务自动化测试，覆盖账号、密码重置与邮箱验证、OAuth 授权码/PKCE/一次性状态、第三方与密码身份关联保护、公开资料隐私与搜索、排行榜隐身、好友关系、同步、成绩、WPM/XP 排行榜、审核引语/资料举报、社区评分、请求限速与维护模式
