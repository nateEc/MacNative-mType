...
密码重置由部署者显式配置的 HTTPS webhook 投递，不绑定特定邮件服务。设置 `TYPEBAR_PASSWORD_RESET_WEBHOOK_URL` 后，服务会向该地址 `POST` JSON 的 `email`、`token` 和 ISO 8601 `expiresAt`；可选的 `TYPEBAR_PASSWORD_RESET_WEBHOOK_TOKEN` 会以 Bearer 令牌放入请求头。webhook 负责把一次性重置码发给用户，切勿记录或转发该码。未配置 webhook 时，能力端点会标为计划中，重置请求会返回明确的 `503`，不会伪称邮件已发出。
...
- 原生账户设置可连接自建服务进行注册、登录、会话恢复、密码重置、资料刷新和退出；访问令牌仅保存在 macOS 钥匙串。重置码仅能使用一次、20 分钟后过期，完成重置会注销该账户所有设备
- 可保存、应用和删除完整测试预设
...
- 44 个自建服务自动化测试，覆盖账号与密码重置、公开资料隐私与搜索、排行榜隐身、好友关系、同步、成绩、WPM/XP 排行榜、审核引语/资料举报、社区评分、请求限速与维护模式
