...
- 再次打开应用会恢复上次有效的测试模式、活动限制、语言、内容规则、引语来源与选择、自定义正文，以及计时／字数／自定义模式各自记住的限制；损坏或未来版本的本机记录会安全回到默认计时测试，恢复默认设置也会清除该记录
...
- 547 个引擎、内容、存储、偏好设置、预设、统计、归档、CSV 导出、账户响应、命令目录及官方布局矩阵单元测试
