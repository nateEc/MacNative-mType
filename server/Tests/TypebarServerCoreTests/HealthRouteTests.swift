...
final class HealthRouteTests: XCTestCase {
  ...
  func testQuoteSubmissionRequiresValidContentAndStartsPending() async throws {
    ...
    XCTAssertEqual(
      ...
      Set([
        submitted.id, spanishSubmission.id, germanSubmission.id, dutchSubmission.id, frenchSubmission.id,
  ...
  func testResultsRejectImpossibleOrMalformedValues() async throws {
    ...
    for (offset, language) in [
      "traditionalChinese", "dutch", "russian", "ukrainian", "ukrainianLatin", "japaneseHiragana",
      "korean",
      "turkish", "polish",
    ].enumerated() {
